<template>
  <div class="card-item buff-card">
    ...
    <div class="card-footer">
      <div class="card-title no-margin" v-if="data?.name">{{ data?.name }}</div>
      <div class="card-description" v-html="data?.description"></div>
      <div class="card-action">
        <span class="card-tag">{{ data?.tag }}</span>
        <a href="#" @click.prevent="viewBuff(item.buffId ? item.buffId : item.id)">{{ item.cta ? item.cta : item.ctaBreaker.cta.ctaText }} 
