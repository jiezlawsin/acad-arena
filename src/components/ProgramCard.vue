<template>
  <div class="card-item buff-card">
    ...
    <div class="card-footer">
      ...
      <div class="card-description" v-html="item.description ? item.description : data?.description"></div>
      <div class="card-action">
        <span class="card-tag">{{ item.tag ? item.tag : data?.tag }}</span>
        <a 
        :href="item.url ? item.url : '#'" 
        @click.prevent="item.url ? goTo(item.url) : viewBuff(item.buffId ? item.buffId : item.id)">{{ item.cta ? item.cta : item.ctaBreaker.cta.ctaText }} 
